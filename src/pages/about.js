  ...
  return (
    <Layout>
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className='w-screen h-screen bg-main-black text-text-light font-bold flex flex-col gap-4 align-left justify-center px-36'>
        <h1 className='font-atkinson-hyperlegible text-5xl uppercase'>
          <Typewriter
            onInit={(typewriter) => {
        ...
        exit={{ y: "100%",  opacity: 0 }}className='font-atkinson-hyperlegible text-5xl'>⠊⠍⠁⠛⠊⠝⠑ ⠞⠓⠑ ⠎⠕⠇⠥⠞⠊⠕⠝⠄⠄⠄</motion.h2>
        <motion.div 
        initial={{ y: "100%", opacity: 0 }}
        animate={{ y: "0%", opacity: 1, transition: { delay: 1.2}}}
        exit={{ y: "100%",  opacity: 0 }}
        className='mt-10 font-archivo uppercase text-xl '>
